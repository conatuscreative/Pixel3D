...
namespace Lidgren.Network
{
    ...
    public partial class NetConnection
    {
        ...
        // Exposed this publically so that I can safely depend on it externally -AR
        public const string ConnectionTimedOutString = "Connection timed out";
        ...
        internal void Heartbeat(float now, uint frameCounter)
        {
            ...
            if ((frameCounter % m_infrequentEventsSkipFrames) == 0)
            {
                if (now > m_timeoutDeadline)
                {
                    ...
                    ExecuteDisconnect(ConnectionTimedOutString, true);
                }
            ...
            if ((frameCounter % m_messageCoalesceFrames) == 0) // coalesce a few frames
            {
                ...
                while (m_queuedOutgoingAcks.Count > 0)
                {
                    ...
                    // write acks header
                    sendBuffer[m_sendBufferWritePtr++] = 0x80; // no sequence number, valid packet, no fragment
