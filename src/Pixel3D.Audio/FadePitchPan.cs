...
namespace Pixel3D.Audio
{
    ...
    public struct FadePitchPan
    {
        ...
        public FadePitchPan(AudioPosition audioPosition)
        {
            var xFade = Math.Abs(audioPosition.x).MapFrom(1.05f, 1.3f).Clamp(); // <- assume horizontal is smaller
            var yFade = Math.Abs(audioPosition.y).MapFrom(1.2f, 1.6f).Clamp();
			...
			if(audioPosition.x < 0)
                pan = -audioPosition.y.MapFrom(-0.4f, -1.2f).Clamp();
            else
                pan =  audioPosition.y.MapFrom( 0.4f,  1.2f).Clamp();
        }
