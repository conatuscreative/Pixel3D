...
namespace Pixel3D.Audio
{
    ...
    public class SafeSoundEffect : IDisposable
    {
        ...
        {
            return owner != null && AudioSystem.playSoundEffect(owner, fpp.fade * SoundEffectVolume, fpp.pitch, fpp.pan);
        }
        ...
        public int DurationInFrames(float pitch)
        {
            ...
            double seconds = AudioSystem.getDuration(owner).TotalSeconds;
        ...
        public static float MasterVolume
        {
	        ...
	        set
            {
	            if (AudioDevice.Available && AudioSystem.getMasterVolume(null) != value) // <- avoid touching native sound engine if we'd just set the same value
