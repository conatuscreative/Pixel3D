...
namespace Pixel3D.Audio
{
	...
	public class SoundRollbackManager
	{
		...
		{
			Debug.Assert(!doingPrediction ||
			             frame > activeFrame); // <- time advances during prediction (time going backwards indicates start of prediction)

			if (rollbackAware == false)
			{
				rollbackAware = true;
				activeFrame = frame; // <- ensure this here, in case we are launching directly into rollback somehow
				liveFrame = frame;
			}
		...
		private readonly List<LiveCue> liveUnmatched = new List<LiveCue>();

		private readonly List<LiveCue> liveCues = new List<LiveCue>();

		private static void CleanupLiveCueList(List<LiveCue> list, int keepFrom)
		{
		...
		private bool TryKillCueExact(Cue cue, int frame, Position? position)
		{
			for (var i = 0; i < liveUnmatched.Count; i++)
			{
		...
		private bool TryKillCueFuzzy(Cue cue, int frame, Position? position)
		{
			...
			for (var i = 0; i < liveUnmatched.Count; i++)
				if (ReferenceEquals(cue, liveUnmatched[i].cue) &&
				    position.HasValue == liveUnmatched[i].playedPosition.HasValue)
				{
					var distanceSq = 0;
					if (position.HasValue)
					{
						distanceSq = Position.DistanceSquared(position.GetValueOrDefault(),
							liveUnmatched[i].playedPosition.GetValueOrDefault());
			...
			if (bestIndex != -1)
			{
				var runningCue = liveUnmatched[bestIndex];
				// Keep these updated:
		...
		private void AddLiveCueNow(Cue cue, Position? position)
		{
			if (rollbackAware)
			{
				LiveCue runningCue;
		...
		private readonly List<PendingCue> pendingCues = new List<PendingCue>();
