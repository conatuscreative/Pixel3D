...
namespace Pixel3D.ActorManagement
{
	public class Actor : StateMachine<UpdateContext>, IHasDrawableFrame, IDrawObject
	{
		public Actor(AnimationSet animationSet)
		{
			...
			currentAnimation = new AnimationPlayer(animationSet.DefaultAnimation);
		}
		...
		{
			public delegate void AnimationTriggersDelegate(Actor self, List<string> triggerList,
		...
		public virtual Actor OutgoingConnection
		{
			get { return null; }
		}

		public virtual Actor IncomingConnection
		{
			get { return null; }
		}
		...
		public void TickAnimation(UpdateContext updateContext)
		{
			...
			var externalAnimationChanged = updateContext.activeActorIndex >= 0 &&
			                               (updateContext.activeActorIndex >= updateContext.initialAnimationStates.Count
			                                || updateContext.initialAnimationStates[updateContext.activeActorIndex] !=
			                                currentAnimation);
			...
			) // If we animate, and we're attached, reattach (if you move yourself later, that's your own problem)
			{
		...
		/// <summary>
		///     Get the world position of an matching incoming attachment point, for an outgoing attachment point, if a valid one
		///     exists.
		///     Does NOT consider if the attachment point is in-range! (Probably prefer <see cref="Character.GrabRangeHelper" />)
		/// </summary>
		public bool GetIncomingAttachment(ref OutgoingAttachmentView outgoingAttachmentView,
			out Position incomingAttachment, List<IncomingAttachmentAttempt> trackIncomingAttachments)
		{
			...
			var animationContext =
				outgoingAttachmentView.attachment.targetAnimationContext.MaybeFlip(
					outgoingAttachmentView.facingLeft != facingLeft);
			var attachmentContext =
				outgoingAttachmentView.attachment.targetAttachmentContext.MaybeFlip(
					outgoingAttachmentView.facingLeft != facingLeft);
			...
			if (animationFrame.incomingAttachments.TryGetBestValue(attachmentContext, out incomingAttachment))
			{
				...
				{
					var inRange = outgoingAttachmentView.attachRange.Contains(incomingAttachment);
					trackIncomingAttachments.Add(new IncomingAttachmentAttempt(outgoingAttachmentView, animation,
						incomingAttachment, inRange));
				}
		...
		{
			var tagFlip = owner.facingLeft != facingLeft;

			var attachmentContext = outgoingAttachment.targetAttachmentContext.MaybeFlip(tagFlip);
			var incomingPosition = currentAnimation.CurrentFrame.incomingAttachments[attachmentContext]
				.MaybeFlipX(facingLeft);
			var outgoingPosition = outgoingAttachment.position.MaybeFlipX(owner.facingLeft);

			position = owner.position + (outgoingPosition - incomingPosition);
		}
		...
		///     mask will appear on-screen. Note that this is only an estimate: gameplay motion can be blocked, animations can
		///     change, etc.
		/// </summary>
