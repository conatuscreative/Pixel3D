...
namespace Pixel3D.ActorManagement
{
	public class UpdateContext : ILocalizationProvider, IAudioPlayer
	{
		...
		public virtual bool DebugNetworkUnsafeHasLocalSettings
		{
			get { return false; }
		}
		...
		/// <summary>
		///     The index in the actor list of the actor currently being updated (or -1 when outside the normal actor update
		///     loop)
		/// </summary>
		...
		public Dictionary<Actor, DeferredAttachment> deferredAttachments =
			new Dictionary<Actor, DeferredAttachment>(ReferenceEqualityComparer<Actor>.Instance);
...
#else
				soundRollbackManager.PlayCueSkipMissingCheck(Definitions, cue, source.position, fpp, parameters, true);
#endif
			}
		}
...
#else
				soundRollbackManager.PlayCueSkipMissingCheck(Definitions, cue, null, fpp, parameters, true);
#endif
			}
		}
...
#else
				soundRollbackManager.PlayCueSkipMissingCheck(Definitions, cue, source, fpp, parameters, true);
#endif
			}
		}
		...
		{
			get { return GameState.Definitions; }
		}
