...
namespace Pixel3D.Pipeline.Audio
{
    public struct SoundParameters
    {
        ...
        {
            SoundParameters result = new SoundParameters();
            ...
            if(File.Exists(path))
            {
                ...
                while(lines.Count > 0)
                {
                    ...
                    {
                        case "loop start":
                            if(!int.TryParse(lines.Dequeue().Trim(), out result.loopStart) || result.loopStart < 0 || result.loopStart >= sampleCount)
                                throw new Exception("ERROR: Bad loop start value (in file " + path + ")");
                            break;

                        case "loop length":
                            if(!int.TryParse(lines.Dequeue().Trim(), out result.loopLength) || result.loopLength < 0 || result.loopLength >= sampleCount)
            ...
            if(result.loopStart != 0 && result.loopLength == 0)
            {
                result.loopLength = sampleCount - result.loopStart;
            }
