...
namespace Pixel3D.Pipeline.Audio
{
	...
	public struct WaveFile
	{
		...
		public static WaveFile Read(BinaryReader br)
		{
			...
			var fileSize = br.ReadInt32(); // <- does not include fileId and fileFormat fields

			var fileFormat = br.ReadUInt32();
			...
			while (!(hasFormatChunk && hasDataChunk))
			{
				...
				var alignedChunkSize =
					chunkSize + (chunkSize & 1); // <- Chunks are word-aligned (round up to nearest word)
				...
				switch (chunkId)
				{
					case fmt_Magic:
					{
						...
						if (result.channelCount != 1 && result.channelCount != 2)
							throw new Exception(
								"Refusing file with too many channels, because XNA won't like it"); // we can handle it, XNA can't

						result.sampleRate = br.ReadInt32();
						if (result.sampleRate < 8000 || result.sampleRate > 48000)
							throw new Exception(
								"Refusing file with unusual sample rate, because XNA won't like it"); // we can handle it, XNA can't
					...
					case dataMagic:
					{
						...
						var debugEnd = br.BaseStream.Position;
