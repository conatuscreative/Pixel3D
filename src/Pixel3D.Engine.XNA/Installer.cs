...
namespace Pixel3D.Engine.XNA
{
    /// <summary>
    /// Links independent Pixel3D engine components to XNA. Should be called once from an external game project.
    /// </summary>
    public static class Installer
    {
        ...
        private static void InstallAudioSystem(int maxPlayers)
        {
            ...
            {
                string c;
                var s = debugContext as string;
                if (s != null)
                    c = s;
                else if (debugContext is IEditorNameProvider)
                {
                    var provider = (IEditorNameProvider)debugContext;
                    c = string.Format("{0}: {1}", provider.GetType().Name, provider.EditorName);
                }
