...
namespace Pixel3D.Engine.LoopSystem
{
    public class MemoryCompareReadStream : Stream
    {
        ...
        {
            this.readBuffer1 = buffer1;
            this.readBuffer2 = buffer2;
            position = 0;
        }

        byte[] readBuffer1, readBuffer2;
        long position;
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                if(readBuffer1[position] != readBuffer2[position])
                {
                    ...
                    byte[] buffer1Result = new byte[count];
                    byte[] buffer2Result = new byte[count];
        ...
        {
            if(position >= readBuffer1.Length || position >= readBuffer2.Length || readBuffer1[position] != readBuffer2[position])
            {
                Debug.Assert(false);
                throw new Exception("Data mismatch at byte " + position);
            }

            byte result = readBuffer1[position];
            position++;
        ...
        public override long Length { get { return Math.Min(readBuffer1.LongLength, readBuffer2.LongLength); } }
